...
def load_photo(path='Foto resmi.JPEG', size=256):
    # Ukuran harus kelipatan 8 agar blok DCT 8x8 pas
...
def generate_binary_watermark(size, seed=42):
    # Seed tetap supaya watermark bisa direproduksi saat ekstraksi
    np.random.seed(seed)
...
def embed_lsb(image_array, watermark):
    # Ganti bit ke-0 channel R dengan bit watermark.
    # Nilai piksel berubah paling banyak ±1, jadi tidak kelihatan secara visual.
...
# --- Metode 2: DCT Mid-Band ---

def _make_dct_matrix():
    # Matriks DCT-II 8x8 sesuai rumus standar JPEG.
    # D[u,x] = C(u) * cos(pi*(2x+1)*u / 16), dengan C(0)=1/sqrt(2), C(u>0)=1
    N = 8
    D = np.zeros((N, N))
    for u in range(N):
...
# Posisi mid-band dalam blok DCT 8x8 — frekuensi menengah yang dipertahankan JPEG
MID_BAND = [(1,2),(2,1),(3,0),(2,3),(1,4),(0,3),(0,5),(1,3)]


def embed_dct(image_array, watermark, alpha=20.0):
    # Konversi ke YCbCr, kerja di channel Y (luminans).
    # Bit watermark diubah ke bipolar (-1/+1) lalu ditambahkan ke koefisien mid-band.
    # Setelah semua blok diproses, konversi balik ke RGB.
    ...
    h, w       = y.shape
    wm_h, wm_w = watermark.shape
    wm_bipolar = (watermark.astype(np.float64) * 2) - 1  # 0 -> -1, 1 -> +1
...
def extract_dct(original_array, watermarked_array, wm_h, wm_w, alpha=20.0):
    # Bandingkan koefisien mid-band antara gambar asli dan watermarked.
    # Selisih positif = bit 1, negatif = bit 0.
    def get_y(arr):
        return np.array(
...
def compress_jpeg(image_array, quality):
    # Pakai BytesIO supaya tidak perlu tulis ke disk
...
def compute_ber(orig_wm, ext_wm):
    # 0.0 = sempurna, 0.5 = sama seperti tebak acak
...
def make_visualization(orig, wm_lsb, wm_dct, watermark,
    ...
    # Baris 0: foto asli, watermark, hasil embed LSB, hasil embed DCT, diff
    ...
    # Baris 1: sampel foto terkompresi di 5 QF berbeda
    show_idx = [0, 3, 6, 9, 13]
    ...
    # Baris 2: hasil ekstraksi watermark DCT di 5 QF
    for col, idx in enumerate(show_idx):
        qf = qf_vals[idx]
    ...
    # Baris 4: perbandingan watermark asli vs hasil ekstraksi
...
def main():
    ...
    print(f"\n>>> Evaluasi kompresi JPEG di {len(qf_values)} nilai QF...")
    for qf in qf_values:
    ...
    # Cetak tabel hasil
    print("\n" + "="*74)
    print("  HASIL EVALUASI WATERMARKING")
    print("="*74)
    ...
    print(f"\nDCT berhasil (BER<0.1) : QF = {ok_dct}")
    print(f"DCT gagal   (BER>0.3) : QF = {fail_dct}")
    print("LSB selalu gagal — JPEG merusak LSB via DCT quantization")
    ...
    print("\nSelesai. Output ada di folder result/")
    print("  - watermarking_evaluation.png")
    print("  - original_face.png")
    print("  - foto_watermarked_lsb.png")
    print("  - foto_watermarked_dct.png")
    print("  - watermark_32x32.png")
